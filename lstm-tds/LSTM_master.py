...
# Data
dat = pd.read_csv('./h1cpudata.csv',engine='python')

split = 0.8
sequence_length = 60

data_prep = LSTM_Prep.Data_Prep(dataset = dat)
rnn_df, validation_df = data_prep.preprocess_rnn(date_colname = 'Timestamp', numeric_colname = 'cpu_usage', pred_set_timesteps = 60)


series_prep = LSTM_Prep.Series_Prep(rnn_df =  rnn_df, numeric_colname = 'cpu_usage')
...
history = model.fit(X_train, y_train, epochs = EPOCHS, validation_split = validation,
          callbacks = [rlrop])
# Loss History
model.save(f'lstm-tda{EPOCHS}.h5')
...
future.predicted_vs_actual(X_min = X_min, X_max = X_max, numeric_colname = 'cpu_usage')
# Predicting 'x' timesteps out
future.predict_future(X_min = X_min, X_max = X_max, numeric_colname = 'cpu_usage', timesteps_to_predict = 15, return_future = True)
